...
@pytest.fixture()
def agents():
    return [Trader("Agent1", 1000, 0, 2, 3, 5, extrapolate_average_profit, price_to_earnings_window=(6,12), trader_volume_risk_aversion= 0.1, propensity_to_switch=1.1),
            Trader("Agent2", 1000, 0, 2, 3, 5, predict_by_moving_avg_growth, price_to_earnings_window=(6,12), trader_volume_risk_aversion= 0.1, propensity_to_switch=1.1),
            Trader("WeirdAgent", 1000, 0, 2, 3, 5, None, price_to_earnings_window=(6,12), trader_volume_risk_aversion= 0.1, propensity_to_switch=1.1)]


def test_adaptive_switching(agents):
    # a none chartist and non fundamentalist agent should raise a value error
    assert_raises(ValueError, adaptive_switching, agents[-1], 0.3, 0.04, 0.03, 8)
